...
void Character::putBlock(VoxelChunk &chunkActuel, BlocDatabase &database) const {
    ...
    if (blocsIntersectes.size() > 0) {
        ...
        //on pose le bloc sur la position adjacente au plus proche
        // on affiche le type de bloc posé
        std::cout << "Bloc posé : " << item->getItemName() << std::endl;
        chunkActuel.setBloc(blocPlusProche.x, blocPlusProche.y, blocPlusProche.z, item->getItemId());
        inventory->removeItem(item->getItemName(), 1);
    }
    //on met à jour le mesh du chunk
    chunkActuel.generateMesh();
    chunkActuel.updateModelMatrix();
    chunkActuel.m_mesh->initializeBuffers();
